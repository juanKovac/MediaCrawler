...
为了让用户更方便地使用 MediaCrawler，我们提供了一个直观的 Web 界面，无需命令行操作即可完成爬虫任务的配置和管理。
...
在浏览器中打开 `http://localhost:5000` 即可访问 Web 界面。
...
- **👥 创作者分析**：专门针对创作者数据的分析页面
  - 创作者信息展示（昵称、粉丝数、IP属地等）
  - 创作者笔记列表查看
  - 笔记详细内容预览（标题、内容、点赞数、收藏数、评论数等）
  - 支持按创作者筛选和查看其所有作品
...
**问题描述**：爬取创作者主页时出现 `[Errno 13] Permission denied: 'data/xhs/1_creator_contents_2025-08-14.csv'` 错误。

**解决方案**：
1. 该问题已在 v1.1.0 版本中修复
2. 修复内容：将异步CSV写入改为同步写入，避免文件锁定问题
3. 增加了权限错误处理机制，当遇到权限问题时会自动创建新文件

**如果仍遇到问题**：
- 确保 `data/xhs/` 目录有写入权限
- 关闭可能占用CSV文件的程序（如Excel）
- 以管理员权限运行程序

### 创作者数据加载错误

**问题描述**：Web界面加载创作者数据时出现 `invalid literal for int() with base 10: '1.7万'` 错误。

**解决方案**：
1. 该问题已在 v1.1.1 版本中修复
2. 修复内容：优化数据处理逻辑，支持包含中文单位的数值格式（如'1.7万'、'2.3千'等）
3. 改进了前端数值显示函数，正确处理已包含中文单位的数值
4. 现在系统可以正常显示和处理中文格式的点赞数、收藏数等数据

**注意事项**：
- 系统现在保持原始的中文数值格式，不再强制转换为纯数字
- Web界面会正确显示'万'、'千'等中文单位
- 如果数据中同时存在纯数字和中文格式，系统会自动适配处理

### 笔记数据加载错误

**问题描述**：点击创作者查看笔记详情时出现 `invalid literal for int() with base 10: '1.7万'` 错误。

**解决方案**：
1. 该问题已在 v1.1.2 版本中修复
2. 修复内容：优化笔记详情数据处理逻辑，支持包含中文单位的数值格式
3. 改进了 `get_creator_notes` 函数，正确处理笔记的点赞数、收藏数等数据
4. 现在笔记列表可以正常显示和处理中文格式的互动数据
...
更多问题解决方案请查看：[docs/常见问题.md](docs/常见问题.md)
